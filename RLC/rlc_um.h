...
public:
    ...
    std::vector<UmdPdu> transmitSdu(const RlcSdu &sdu, size_t segmentSize);
...
public:
    ...
    ~ReceivingUmRlcEntity();
    RlcSdu receivePdu(const UmdPdu &pdu);
...
private:
    ...
    bool checkCompleteSdu(uint16_t sn, RlcSdu &sdu);
