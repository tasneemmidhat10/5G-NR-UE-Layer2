...
public:
    ...
    static std::vector<uint8_t> generateAESCmac(std::vector<uint8_t> &data, const uint8_t *key);
};
...
public:
    ...
    std::vector<uint8_t> verifyAESCmac(const std::vector<uint8_t> &data, const uint8_t *key, const uint8_t *receivedMac = nullptr);
